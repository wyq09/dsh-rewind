...
给 **DeepSeek Harness（DSH）** 用的「检查点 / 回退」标准插件（Profile Bundle），参考 [arpagon/pi-rewind](https://github.com/arpagon/pi-rewind) 改写。
...
AI 帮你写代码时偶尔会改错文件、删错东西。这个插件在每个回合结束时自动把工作区存成一个检查点，你想回到某一步时，先看 diff（会改哪些文件），确认后再一键还原，还原错了还能撤销。
...
> 需要已安装 DeepSeek Harness（`dsh`），适用于 `web` profile。目标机器要有 `git`（以及 `mkdir` / `tee` / `cat` / `rm`，macOS 和 Linux 一般都有）。
...
把本仓库放到 `~/.dsh/profiles/web/dsh-rewind`，然后：

1. 在 `~/.dsh/profiles/web/package.json` 的 `dependencies` 加 `"dsh-rewind": "file:./dsh-rewind"`
2. 在 `dsh.profile.bundles` 列表追加 `"dsh-rewind"`
3. 重启 `dsh web`

重启后打开页面，**按 Ctrl+F5 强制刷新**，右下角即可看到 `⏪` 悬浮胶囊。
...
- 点它展开时间线面板：
  - **检查点列表**（最新的在最上面）：`T+回合号`、触发类型、时间、文件数、标签
  - **点某一行** → 下方显示 **diff 预览**
  - 按钮：`📸 立即快照` / `↺ 还原选中项`（**点两下**才执行，第二下变红色确认键）/ `↩ 撤销` / `↪ 重做`
  - 标题栏 `⟳` 刷新 / `−` 收起 / `×` 关闭
...
├── cordis.patch.yml    # bundle 挂载声明（insert 一个 host 行）
├── dsh/
│   └── index.js        # 后端：git 检查点引擎 + HTTP 端点 + rewind 工具
└── client/
    └── client.js       # 前端：shell.overlay 悬浮时间线面板
```
